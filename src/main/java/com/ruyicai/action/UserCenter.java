...
public class UserCenter extends BaseAction {
	...
	public String loginTouch() {
		...
		try {
			...
			String userName = StringUtils.isBlank(request.getParameter("username")) ? "" : request
	...
	public void loginForUserno() {
		...
		try {
			...
			String userInfoStr = JSONObject
					.fromObject(
							JSONReslutUtil.getResultMessage(USER_LINKURL + "/tuserinfoes?",
	 ...
	 * 例子：http://users.boyacai.com/user/center!getUserNo;jsessionid=｛cookie中存储的值｝?a=随机数
	...
	/**
	 * 通过http请求可以修改当前用户的信息
	 * 例子：http://users.boyacai.com/user/center!logout;jsessionid=｛cookie中存储的值｝?a=随机数
	 * http://users.boyacai.com/user/center!logout;jsessionid=D051904206F696879C5C507444217FFD?a=3493537239827349
	 */
	public void logout() {
