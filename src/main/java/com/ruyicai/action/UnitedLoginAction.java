...
public class UnitedLoginAction extends BaseAction {
	...
	public String alipayHandyLogin() {
		...
		try {
			...
			//2.生成支付宝快捷登录的待签名字符串
			String[] params = new String[] { "partner=" + ResourceBundleUtil.ALIPAY_ID,
					"target_service=user.auth.quick.login", "service=alipay.auth.authorize", "_input_charset=utf-8",
	...
	public String alipayProLogin(String token) {
		...
		//2.生成支付宝快捷登录的待签名字符串
		String[] params = new String[] { "partner=" + ResourceBundleUtil.ALIPAY_ID,
				"service=user.logistics.address.query", "_input_charset=utf-8", "token=" + token,
				"return_url=" + return_url };
		Arrays.sort(params);//从a-z排序
		StringBuilder builder = new StringBuilder();
		//切割用&符号链接
		...
		try {
			...
			if ("".equals(session.getAttribute("flag")) || session.getAttribute("flag") == null) {
				...
				String re = JSONReslutUtil.getResultMessage(ResourceBundleUtil.LINKURL
						+ "/tbiguserinfoes?json&find=BigUser&outuserno=" + session.getAttribute("user_id") + "&type="
						+ alipayType);
			...
			logger.info("得到支付宝返回的user_id=" + session.getAttribute("user_id") + ";token="
					+ session.getAttribute("token") + ";real_name=" + session.getAttribute("name") + ";email="
					+ session.getAttribute("email") + ";cert_no=" + cert_no + ";mobile=" + mobile);
			...
			if (CommonUtil.getBackValue("errorCode", obj).equals(LotErrorCode.NEW_OK)) {
				...
				response.getWriter()
						.print("<script language='javascript'>window.location.href='/unitedLogin/setNickName_logistics.jsp'</script>");
			}
			return null;
	...
	public String userInfoBind() {
		...
		try {
			...
			//用户执行大客户绑定 
			String bindRe = getUserBindInfo(tuser.getUSERNO(), (String) session.getAttribute("type"),
	...
	public String qqUnitedHandlyLogin() {
		...
		try {
			...
			String[] params = new String[] { "response_type=code", "client_id=" + qq_app_id,
	...
	public String qqUnitedLogin() {
		...
		try {
			...
			String[] tokenparams = new String[] { "grant_type=authorization_code", "client_id=" + qq_app_id,
					"client_secret=" + qq_app_key, "code=" + value, "state=" + state,
			...
			String userinfoparams = "access_token=" + access_token + "&oauth_consumer_key=" + oauth_key + //+qq_app_key+
					"&openid=" + openId;
	...
	public String getUserBindInfo(String userno, String type, String outuserno) throws Exception {
		...
		String re = JSONReslutUtil.getResultMessage(ResourceBundleUtil.LINKURL + "/tbiguserinfoes/bind?", "userno="
				+ userno + "&type=" + type + "&outuserno=" + outuserno, "POST");
	...
	public String byBindUserInfo(JSONObject obj) throws Exception {
		...
		JSONObject userinfoes = JSONObject.fromObject(JSONReslutUtil.getResultMessage(ResourceBundleUtil.LINKURL
	...
	public String registerUser() {
		...
		try {
			...
			String inputyzm = request.getParameter("inputyzm") == null
					|| request.getParameter("inputyzm") == "undefind" ? "" : request.getParameter("inputyzm")
					.toString();
			...
			if (request.getParameter("msie") != null) {
				if (request.getParameter("msie").equals("1") && !"中文".equals(isIe)) {
					username = request.getParameter("username").trim() == "undefined" ? "" : request
			...
			String real_name = (String) (request.getSession().getAttribute("name") == null ? " " : request.getSession()
					.getAttribute("name"));
			String card_id = (String) (request.getSession().getAttribute("cert_no") == null
					|| request.getSession().getAttribute("cert_no") == "" ? "111111111111111111" : request.getSession()
					.getAttribute("cert_no"));
			String mobile = (String) request.getSession().getAttribute("mobile");

			//调用新接口执行注册
			String params = "accesstype=" + Constant.WEB_AGENCYNO + "&agencyno="
					+ CommonUtil.processChannelId(agencyNo == null ? "000000" : agencyNo) + "&channel=" + channel
					+ "&info=ruyicai&leave=1&state=&userName=" + username + "&password=" + password + "&name="
					+ real_name + "&certid=" + card_id;
			if (email != null && email.indexOf("@") > 0) {
				params = params + "&email=" + email;
			}
			if (mobile != null) {
				params = params + "&mobileid=" + mobile;
			}
			params = params + "&type=" + type;
			String re = JSONReslutUtil.getResultMessage(ResourceBundleUtil.LINKURL + "/tuserinfoes/register?", params,
					"POST");
