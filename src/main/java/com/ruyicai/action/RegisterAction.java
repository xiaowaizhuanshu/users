...
public class RegisterAction extends BaseAction {
	...
	public String new_register() throws IOException {
		...
		try {
			...
			*/
			...
			// 执行新接口注册
			String reUser = JSONReslutUtil.getResultMessage(ResourceBundleUtil.LINKURL + "/tuserinfoes?",
					"json&find=ByUserName&userName=" + username, "POST");
			String re = "";
			...
			if (reUser != null) {
				...
				if (jsonReUser != null && !jsonReUser.isNullObject()) {
					if ("0".equals(jsonReUser.getString("errorCode"))) {
						...
						if (jsonReUser.getJSONObject("value").getInt("state") == 0
								|| jsonReUser.getJSONObject("value").getInt("state") == 2) {
							Tuserinfo userinfo = Tuserinfo.setJson(jsonReUser.getJSONObject("value"));
							re = JSONReslutUtil.getResultMessage(
									ResourceBundleUtil.LINKURL + "/tuserinfoes/modify?",
									"userno=" + userinfo.getUSERNO() + "&accesstype=" + Constant.WEB_AGENCYNO
											+ "&agencyno="
											+ CommonUtil.processChannelId(agencyNo == null ? "000000" : agencyNo)
											+ "&certid=" + ResourceBundleUtil.CARDID + "&state=1"
											+ "&info=ruyicai&leave=1" + "&nickname=" + " " + "&password="
											+ URLEncoder.encode(password) + "&type=1", "POST");
							isRegister = false;
						}
			...
			if (isRegister) {
				re = JSONReslutUtil.getResultMessage(
						ResourceBundleUtil.LINKURL + "/tuserinfoes/register?",
						"accesstype=" + Constant.WEB_AGENCYNO + "&agencyno="
								+ CommonUtil.processChannelId(agencyNo == null ? "000000" : agencyNo) + "&certid="
								+ ResourceBundleUtil.CARDID + "&channel=" + channel + "&info=ruyicai&leave=1"
								+ "&nickname=" + " " + "&password=" + URLEncoder.encode(password) + "&userName="
								+ username + "&type=1", "POST");
			}
			...
			if (obj.getString("errorCode").equals(LotErrorCode.NEW_OK)) {
				...
				if (userid != null) {
					...
					String reagencyUrl = JSONReslutUtil.getResultMessage(agencyUrl + "/createUserAgency?",
							"parentUserno=" + parentUserno + "&userno=" + user.getUSERNO(), "POST");
	...
	public String wapRegister() {
		...
		String channel = request.getParameter("channel") == null || request.getParameter("channel").equals("") ? "1"
				: request.getParameter("channel");//channel is 1
		...
		try {
					...
					if ("0".equals(jsonReUser.getString("errorCode"))) {
						...
						if (jsonReUser.getJSONObject("value").getInt("state") == 0
							...
							re = JSONReslutUtil.getResultMessage(ResourceBundleUtil.LINKURL + "/tuserinfoes/modify?",
									"userno=" + userinfo.getUSERNO() + "&accesstype=W" + "&certid="
											+ ResourceBundleUtil.CARDID + "&state=1" + "&info=wap&leave=1"
			...
			if (isRegister) {
				...
				re = JSONReslutUtil.getResultMessage(ResourceBundleUtil.LINKURL + "/tuserinfoes/register?", parameter,
						"POST");
			}
			JSONObject obj = JSONObject.fromObject(re);
			if (obj.getString("errorCode").equals(LotErrorCode.NEW_OK)) {
